...
export default function Page() {
  ...
  const runAction = async (action: 'sms' | 'remark') => {
    ...
    for (let i = 0; i < rows.length; i++) { const row = rows[i]; const orderNum = String(row.orderNum || ''); if (!orderNum) continue; setStatus(`${action === 'sms' ? 'Sending SMS' : 'Saving remarks'} ${i + 1} of ${rows.length}...`); try { const payload = action === 'sms' ? { orderNum, templateId: template, country: row.country || 'NG', appName: row.appName || '' } : { orderNum, reachOutBy, contactRelations, remark: remark || null, contactName: contactName || null, contactResult, collectionTag, contactNo: contactNo || null }; await kimbo(action === 'sms' ? '/adminApi/system/loan/collectionAssign/sendSms' : '/adminApi/system/loan/collectionRecord', token, 'POST', payload); logs.push(`SUCCESS ${orderNum}`) } catch (e) { logs.push(`FAILED ${orderNum}: ${e instanceof Error ? e.message : 'Unknown error'}`) } }
  ...
  return <main className="min-h-screen bg-muted/30 pt-[76px] text-foreground sm:pt-[116px]"><header className="fixed inset-x-0 top-0 z-40 border-b bg-background/95 shadow-sm backdrop-blur"><div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-3 py-3 sm:px-6 sm:py-4"><div className="min-w-0"><p className="truncate text-[10px] font-semibold uppercase tracking-[0.16em] text-primary sm:text-xs sm:tracking-[0.2em]">Empathy Desk Workspace</p><h1 className="truncate text-lg font-semibold tracking-tight sm:text-2xl">Empathy Desk Workspace</h1></div><div className="flex shrink-0 items-center gap-2"><div className="hidden min-w-0 items-center gap-2 text-xs text-muted-foreground sm:flex"><Wifi className="size-4 shrink-0" /><span className="max-w-52 truncate">{status}</span></div><div className="hidden items-center gap-1 rounded-lg border bg-muted/30 p-1 sm:flex">{nav.map(([key, label]) => <button key={key} onClick={() => setTab(key)} className={`rounded-md px-2.5 py-2 text-xs font-medium transition-colors lg:px-3 ${tab === key ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-background hover:text-foreground'}`}>{label}</button>)}</div><Button className="sm:hidden" variant="outline" size="icon" aria-label={menuOpen ? 'Close navigation menu' : 'Open navigation menu'} aria-expanded={menuOpen} onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? <X data-icon="inline-start" /> : <Menu data-icon="inline-start" />}</Button></div></div>{menuOpen && <nav className="mx-3 mb-3 grid gap-1 rounded-lg border bg-muted/30 p-2 shadow-sm sm:hidden">{nav.map(([key, label]) => <button key={key} onClick={() => { setTab(key); setMenuOpen(false) }} className={`rounded-md px-3 py-2.5 text-left text-sm font-medium transition-colors ${tab === key ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-background hover:text-foreground'}`}>{label}</button>)}</nav>}</header>
